...
export interface Analysis { decisions: Decision[]; critical: Decision[]; engine: string; version: number; note: string }
export interface Game { id: string; fen: string; initialFen: string; moves: string[]; version: number; playerColor: Color; elo: number; turn: Color; title: string; pgn: string; legalMoves: string[]; result: string | null; source: string; analysis: Analysis | null; drill: {name:string; goal:string; theme:string; target:number; decisions:number; startPly:number; complete:boolean} | null }
