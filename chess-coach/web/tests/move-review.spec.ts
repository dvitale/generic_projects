...
test('Review compares every move before it is played and uses the selected Maia level',async({page,request})=>{
  ...
  await review.getByText('Varianti verificate da Stockfish').click()
  for(const size of [{width:1920,height:1080},{width:1440,height:900},{width:1280,height:800}]){
  ...
  await page.screenshot({path:'test-results/review-wide.png',fullPage:true})
  await page.getByRole('tab',{name:'Elo',exact:true}).click()
  await expect(review).not.toBeVisible()
  await expect(page.getByRole('region',{name:'Valutazione Elo della partita'})).toBeVisible()
  await page.getByRole('tab',{name:'Elo',exact:true}).press('ArrowRight')
  await expect(page.getByRole('tab',{name:'Tutor',exact:true})).toBeFocused()
  await page.getByLabel('Il tuo ragionamento').fill('Riflessione conservata cambiando scheda.')
  await page.getByRole('tab',{name:'Mosse',exact:true}).click()
  await expect(review.getByRole('combobox',{name:'Mossa da confrontare'})).toHaveValue('4')
  await page.getByRole('tab',{name:'Tutor',exact:true}).click()
  await expect(page.getByLabel('Il tuo ragionamento')).toHaveValue('Riflessione conservata cambiando scheda.')
  await page.getByRole('tab',{name:'Mosse',exact:true}).click()
